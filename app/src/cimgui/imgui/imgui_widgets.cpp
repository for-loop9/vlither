...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    const float cross_thickness = 2.0f; // FIXME-DPI
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    if (style.FrameBorderSize != 0) {
        window->DrawList->AddRect(grab_rect.Min + ImVec2(1, 0), grab_rect.Max, GetColorU32(ImGuiCol_BorderShadow), style.ScrollbarRounding);
        window->DrawList->AddRect(grab_rect.Min, grab_rect.Max, GetColorU32(ImGuiCol_Border), style.ScrollbarRounding);
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        {
            const float pad = ImMax(1.0f, IM_TRUNC(square_sz / 4.5f));
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    {
        window->DrawList->AddCircle(center + ImVec2(0, 1), radius - 1, GetColorU32(ImGuiCol_BorderShadow), num_segment, style.FrameBorderSize);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(grab_bb.Min + ImVec2(0, 1), grab_bb.Max - ImVec2(0, 1), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        if (style.FrameBorderSize != 0) {
            window->DrawList->AddRect(grab_bb.Min + ImVec2(0, 2), grab_bb.Max - ImVec2(0, 1), GetColorU32(ImGuiCol_BorderShadow), style.GrabRounding);
            window->DrawList->AddRect(grab_bb.Min + ImVec2(0, 1), grab_bb.Max - ImVec2(0, 1), GetColorU32(ImGuiCol_Border), style.GrabRounding);
        }
    }
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.y > grab_bb.Min.y) {
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
         if (style.FrameBorderSize != 0) {
            window->DrawList->AddRect(grab_bb.Min + ImVec2(0, 1), grab_bb.Max, GetColorU32(ImGuiCol_BorderShadow), style.GrabRounding);
            window->DrawList->AddRect(grab_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_Border), style.GrabRounding);
        }
    }
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    else
    {
        ...
        if (ArrowButtonEx("-", ImGuiDir_Left, ImVec2(button_size, button_size)))
        {
            DataTypeApplyOp(data_type, '-', p_data, p_data, g.IO.KeyCtrl && p_step_fast ? p_step_fast : p_step);
            value_changed = true;
        }
        SameLine(0, style.ItemInnerSpacing.x);
        if (ArrowButtonEx("+", ImGuiDir_Right, ImVec2(button_size, button_size)))
        {
            DataTypeApplyOp(data_type, '+', p_data, p_data, g.IO.KeyCtrl && p_step_fast ? p_step_fast : p_step);
            value_changed = true;
        }
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    if (g.Style.TabBorderSize > 0.0f)
    {
        ...
        draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f + 1), rounding, 6, 9);
        draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f + 1), rounding, 9, 12);
        draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, y2));
        draw_list->PathStroke(GetColorU32(ImGuiCol_BorderShadow), 0, g.Style.TabBorderSize);
